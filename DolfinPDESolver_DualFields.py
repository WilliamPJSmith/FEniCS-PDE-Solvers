...
class DolfinSolver:
	...
	def __init__(self, solverParams):
		...
		self.Ka = solverParams['Ka']
		self.mu_effa = solverParams['mu_effa'] 
		self.Kb = solverParams['Kb']
		self.mu_effb = solverParams['mu_effb'] 
	...
	def SolvePDE(self, centers_A, centers_B, vols, filename, dir, stepNum=0):
		"""
		...
		self.GetVolFracs_A(centers_A, vols)
		self.GetVolFracs_B(centers_B, vols)
	...
	def SolveDoublePDE(self, centers, vols, filename, dir, stepNum=0):
		"""
		High-level function to be called during the function.
		Features 2 independent fields. Added HT2016.
		"""
		...
		# processes common to both fields
		self.mesh = self.DoubleBufferedMesh(max_height)
		...
		F1 = dot(grad(u), grad(p))*dx + Da*(u / (u+Ka))*(v / (v+Kb))*self.Phi()*p*dx 
		F2 = dot(grad(v), grad(q))*dx + Db*(u / (u+Ka))*(v / (v+Kb))*self.Phi()*q*dx
		F = F1 + F2	
		
		# set boundary conditions
		tbc = TopDirichletBoundary()
		bbc = BaseDirichletBoundary()
		bc0 = DirichletBC(self.V, Constant((self.u0, 0.0)), tbc)
		bc1 = DirichletBC(self.V.split()[1], Constant(self.u0), bbc)
		bcs = [bc0,bc1]
		...
		# make the solution equal to each of the two fields in turn to use the interpolater
		self.solution = W.split()[0]
		u_local_a = self.InterpolateToCenters(centers)
		if stepNum % self.pickleSteps == 0:
			self.WriteFieldToFile(dir+filename+'_a.pvd', self.solution)
			
		self.solution = W.split()[1]
		u_local_b = self.InterpolateToCenters(centers)
		if stepNum % self.pickleSteps == 0:
			self.WriteFieldToFile(dir+filename+'_b.pvd', self.solution)
	
		return u_local_a, u_local_b
	...
	def MonodNutrientSink_A(self, u):
		""" 
		Monod function with which to build RHS.
		"""
		a = Constant(self.mu_effa)
		b = Constant(self.K)
		
		return -1 * a * u * VolumeFraction_A() / (b + u)
	...
	def MonodNutrientSink_A(self, u):
		...
		Monod function with which to build RHS.
		"""
		a = Constant(self.mu_effb)
		b = Constant(self.K)
		
		return -1 * a * u * VolumeFraction_B() / (b + u)
